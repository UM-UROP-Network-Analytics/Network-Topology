        ...
        if rt_src in src_to_dest.keys():
            if rt_dest in src_to_dest[rt_src].keys():
                ...
                if (dupe_rt == 0):
                    ...
                    cur.execute("INSERT INTO test1 (src, dest, rtnum, count, hops) VALUES (rt_src, rt_dest, current_rt[2:], 1, rt_hops)")
                    conn.commit()
                    print("Insert 1")
            else:
                ...
                cur.execute("INSERT INTO test1 (src, dest, rtnum, count, hops) VALUES (rt_src, rt_dest, 1, 1, rt_hops)")
                conn.commit()
                print("Insert 2")
            dupe_rt = 0
